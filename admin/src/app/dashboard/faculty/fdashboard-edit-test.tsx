...
export default function EditTestPage() {
  ...
  const fetchQuestionBank = useCallback(
    async (type: "mcq" | "coding" | "assignment") => {
      try {
  ...
  useEffect(() => {
    ...
    const allSets = [
      ...
      ...questionBankMap.coding.map((set) => ({
        ...set,
        type: "coding" as const,
      })),
      ...questionBankMap.assignment.map((set) => ({
        ...set,
    ...
    const addToSelection = (ids: string[]) => {
      ids.forEach((id) => {
        allSets.forEach((set) => {
          if (set.questions?.some((q) => (q.question_id || q._id) === id)) {
      ...
      questionBankMap[type].forEach((set) => {
        set.questions?.forEach((question) => {
          ...
          if (questionId && customQuestion) {
            initialEdited[makeQuestionKey(type, set._id, questionId)] =
              normalizeQuestion(customQuestion, questionId);
          }
        });
      });
    });
  ...
  const updateEditingTestCase = (
    ...
    setEditingQuestion((current) => {
      if (!current) return current;
      const testCases = current.draft.test_cases.map(
  ...
  const removeEditingTestCase = (index: number) => {
    setEditingQuestion((current) => {
      ...
      return {
        ...
        draft: {
          ...current.draft,
          test_cases: current.draft.test_cases.filter(
    ...
    getAllowedQuestionTypes(formData.type).forEach((type) => {
      questionBankMap[type].forEach((set) => {
        ...
        set.questions.forEach((question) => {
          ...
          const draft =
            editedQuestions[key] || normalizeQuestion(question, questionId);
        ...
        marksAllocation: {
          mcq:
            formData.type === "assignment"
              ? 0
              : formData.marksAllocation?.mcq || 0,
          coding:
            formData.type === "assignment"
              ? 0
              : formData.marksAllocation?.coding || 0,
          assignment:
            formData.type === "assessment"
              ? 0
              : formData.marksAllocation?.assignment || 0,
        },
        settings:
          formData.type === "assignment"
            ? {
            ...
            : formData.settings,
        instructions:
          formData.type === "assignment" ? "" : formData.instructions || "",
      };
      ...
      questionBankMap[type].forEach((set) => {
        ...
        set.questions.forEach((q) => {
          ...
          if (selected.includes(qid)) {
            ...
            result[type].push(
              editedQuestions[key] || normalizeQuestion(q, qid),
            );
          }
        });
      });
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
            ...
            <input
              ...
              value={
                formData.scheduledStart
                  ? toLocalDatetimeString(formData.scheduledStart)
                  : ""
              }
            ...
            <input
              ...
              value={
                formData.scheduledEnd
                  ? toLocalDatetimeString(formData.scheduledEnd)
                  : ""
              }
      ...
      <div className="bg-neutral-800 rounded-lg border border-neutral-700 p-6 space-y-4">
        ...
        <div
          className={`grid grid-cols-1 gap-4 ${selectedTestType === "assignment" ? "md:grid-cols-2" : selectedTestType === "assessment" ? "md:grid-cols-3" : "md:grid-cols-4"}`}
        >
          {showMcqMarks && (
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">
              ...
              <input
                ...
                onChange={(e) =>
                  handleMarksChange("mcq", parseInt(e.target.value))
                }
                min="0"
                className="w-full px-4 py-2 bg-neutral-900 text-gray-100 border border-neutral-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
          ...
          {showCodingMarks && (
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  handleMarksChange("coding", parseInt(e.target.value))
                }
                min="0"
                className="w-full px-4 py-2 bg-neutral-900 text-gray-100 border border-neutral-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
          ...
          {showAssignmentMarks && (
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  handleMarksChange("assignment", parseInt(e.target.value))
                }
                min="0"
                className="w-full px-4 py-2 bg-neutral-900 text-gray-100 border border-neutral-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                handleChange("passingPercentage", parseInt(e.target.value))
              }
              min="0"
              max="100"
              className="w-full px-4 py-2 bg-neutral-900 text-gray-100 border border-neutral-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <label className="flex items-center gap-3 p-3 border border-neutral-700 rounded-lg hover:bg-neutral-700 cursor-pointer">
              <input
                ...
                onChange={(e) =>
                  handleSettingsChange("allowLateSubmission", e.target.checked)
                }
            ...
            <label className="flex items-center gap-3 p-3 border border-neutral-700 rounded-lg hover:bg-neutral-700 cursor-pointer">
              <input
                ...
                onChange={(e) =>
                  handleSettingsChange("enableProctoring", e.target.checked)
                }
                className="w-4 h-4 text-blue-600"
              />
              <span className="text-sm font-medium text-gray-300">
                    ...
                    return (
                      ...
                      >
                        <input
                          type="checkbox"
                          checked={checked}
                          onChange={() => toggleQuestionSelection(set._id, qid)}
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="break-words">
                            Q{index + 1}.{" "}
                            {editedQuestion?.question_text || q.question_text}
                          </p>
            ...
            <div className="space-y-5">
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    updateEditingDraft({ question_text: e.target.value })
                  }
                  rows={4}
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="space-y-3">
                      {editingQuestion.draft.options.map(
                        (option, optionIndex) => (
                          <input
                            key={optionIndex}
                            type="text"
                            value={option}
                            onChange={(e) =>
                              updateEditingOption(optionIndex, e.target.value)
                            }
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateEditingDraft({ correct_answer: e.target.value })
                      }
                      className="w-full px-4 py-2 bg-neutral-800 text-gray-100 border border-neutral-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
              ...
              {editingQuestion.type === "coding" && (
                <div>
                  ...
                  <div className="space-y-4">
                    {editingQuestion.draft.test_cases.map(
                      (testCase, testCaseIndex) => (
                        <div
                          key={testCaseIndex}
                          ...
                          <div className="flex items-center justify-between gap-3">
                            <label className="flex items-center gap-2 text-sm text-gray-300">
                              <input
                                ...
                                onChange={(e) =>
                                  updateEditingTestCase(testCaseIndex, {
                            ...
                            <button
                              type="button"
                              onClick={() =>
                                removeEditingTestCase(testCaseIndex)
                              }
