...
export default function FacultyDashboard() {
  ...
  const fetchFaculty = async () => {
    try {
      ...
      const data = await getUsersByRole("faculty");
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {loading ? (
          <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-10 text-center">
            ...
            <p className="text-neutral-400">Loading faculty data...</p>
          </div>
        ) : faculty.length === 0 ? (
          <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-10 text-center">
            <p className="text-neutral-500 text-lg">No faculty members found</p>
          </div>
        ) : (
          <UserTable users={faculty} onRefresh={fetchFaculty} />
        )}
      </div>
    </div>
  );
