...
import {
  getMyAssignmentAttempt,
  getMyAssignmentDownloadUrl,
  getTestById,
  submitAssignment,
} from "@/apis/test-api";
...
export const AssignmentDetail = () => {
  ...
  const handleViewSubmittedPdf = async () => {
    if (!assignmentId || viewingPdf) return;

    try {
      setViewingPdf(true);
      const response = await getMyAssignmentDownloadUrl(assignmentId);
      const signedUrl =
        response.data?.data?.signedUrl || attempt?.responses?.assignmentFileUrl;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const err = error as {
        response?: { data?: { message?: string; data?: AssignmentAttempt } };
      };
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            <div>
              ...
              {attempt.responses.assignmentFileUrl && (
                <button
                  type="button"
                  onClick={handleViewSubmittedPdf}
                  disabled={viewingPdf}
                  className="mt-3 inline-flex items-center gap-2 text-sm font-medium text-orange-400 hover:text-orange-300 disabled:opacity-60"
                >
                  <Download size={16} />
                  {viewingPdf ? "Opening PDF..." : "View submitted PDF"}
                </button>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:min-w-96">
              <div className="rounded-lg border border-neutral-700 bg-neutral-900 p-4">
                ...
                <p className="text-2xl font-bold text-neutral-100">
                  ...
                  {attempt.score.total || assignment.totalMarks}
                </p>
                <p className="text-sm text-neutral-400">
      ...
      {/* Upload Section */}
      <div
        className={`bg-neutral-800 rounded-xl border-2 border-dashed p-8 ${hasSubmittedAssignment ? "border-green-700" : "border-neutral-600"}`}
      >
        ...
        {hasSubmittedAssignment ? (
          <div className="flex flex-col items-center justify-center py-8 text-center">
            ...
            <p className="text-neutral-100 font-medium">
              Your PDF has been uploaded.
            </p>
            <p className="text-sm text-neutral-400 mt-2">
              Only one submission is allowed for this assignment.
            </p>
          </div>
        ) : !uploadedFile ? (
