...
export const AssessmentLayout = () => {
    ...
    const questions: Question[] = [
        { id: 'q_code_001', type: 'coding', title: 'Factorial using Recursion', section: 'Coding' },
        { id: 'q_code_002', type: 'coding', title: 'Palindrome Checker', section: 'Coding' },
        { id: 'q_mcq_001', type: 'mcq', title: 'Binary Search Complexity', section: 'MCQ' },
        { id: 'q_mcq_002', type: 'mcq', title: 'Stack Data Structure', section: 'MCQ' },
        { id: 'q_code_003', type: 'coding', title: 'Reverse Linked List', section: 'Coding' },
    ...
    const currentMcqIndex = mcqQuestions.findIndex(q => q.id === currentQuestionId);
    
    const handleNext = () => {
        if (currentQuestion?.type === 'mcq' && currentMcqIndex < mcqQuestions.length - 1) {
    ...
    const showNext = currentQuestion?.type === 'mcq' && currentMcqIndex < mcqQuestions.length - 1;
    ...
    return (
        <div className='relative flex flex-col h-screen w-full overflow-hidden bg-neutral-950'>
            ...
            <main className="m-2 relative flex flex-row flex-1 gap-2 overflow-hidden">
                <aside className={`relative rounded-md border border-neutral-800 bg-neutral-900 h-full overflow-y-auto transition-all duration-300 ease-in-out ${sidebarOpen ? "w-72" : "w-16"}`}>
                    {sidebarOpen ? (
                        <div className="p-4 space-y-4">
                            ...
                            <div>
                                <div className="flex items-center gap-2 mb-3">
                                    ...
                                    <h3 className="text-sm font-semibold text-neutral-300">Coding Problems</h3>
                                </div>
                                <div className="space-y-2">
                                    {questions.filter(q => q.type === 'coding').map((question, idx) => {
                            ...
                            <div>
                                <div className="flex items-center gap-2 mb-3">
                                    ...
                                    <h3 className="text-sm font-semibold text-neutral-300">Multiple Choice</h3>
                                </div>
                                <div className="grid grid-cols-5 gap-2">
                                    {questions.filter(q => q.type === 'mcq').map((question, idx) => {
                                        const isAnswered = !!answers[question.id];
                                        return (
                                            <button
                                                key={question.id}
                            ...
                            {questions.map((question) => (
                                <button
                                    ...
                                    className={`w-full p-2 rounded-lg transition-all duration-200 ${
                                        currentQuestionId === question.id
                                            ? question.type === 'coding' ? 'bg-blue-600' : 'bg-green-600'
                ...
                >
                    <PanelRightOpen className={`h-4 w-4 transition-transform duration-200 ${sidebarOpen ? "rotate-0" : "rotate-180"}`}
                    />
                </div>
