...
const buildAssessmentAccess = async (test, studentId) => {
  ...
  if (!studentId || test.type !== "assessment") {
    return {
      hasSession: false,
      isBlocked: false,
      status: "not_started",
  ...
  if (
    ...
    session.status = "time_over";
    session.lockReason = "time_over";
  ...
  const remainingSeconds = session.endsAt
    ? Math.max(0, Math.ceil((session.endsAt.getTime() - now.getTime()) / 1000))
    : durationSeconds;
  const isBlocked =
    session.status === "submitted" || session.status === "time_over";

  return {
    hasSession: true,
    isBlocked,
    status: session.status,
    timeLimitMinutes: session.timeLimitMinutes || durationMinutes,
    remainingSeconds,
...
export const getTests = async (req, res) => {
  try {
    const { status, type, subjectId, page = 1, limit = 10, search } = req.query;
  ...
  try {
    ...
    if (testWithQuestions.customQuestions?.coding?.length > 0) {
      testWithQuestions.codingQuestions =
        testWithQuestions.customQuestions.coding;
    ...
    if (testWithQuestions.customQuestions?.assignment?.length > 0) {
      testWithQuestions.assignmentQuestions =
        testWithQuestions.customQuestions.assignment;
...
export const getPublicTestById = async (req, res) => {
  try {
    ...
    if (testWithQuestions.customQuestions?.coding?.length > 0) {
      ...
      testWithQuestions.codingQuestions = pickSelectedQuestions(
        codingDocs,
        codingIds,
      );
    } else {
    ...
    if (testWithQuestions.customQuestions?.assignment?.length > 0) {
      ...
      testWithQuestions.assignmentQuestions = pickSelectedQuestions(
        assignmentDocs,
        assignmentIds,
      );
    } else {
    ...
    testWithQuestions.assessmentAccess = await buildAssessmentAccess(
      test,
      req.user?.userId,
    );
...
export const startPublicAssessmentSession = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    if (access.isBlocked) {
      return res.status(409).json({
        success: false,
        message: "Assessment session is already closed",
        data: access,
      });
    }
    ...
    if (!session) {
      ...
      const endsAt = new Date(
        startedAt.getTime() + (test.durationMinutes || 0) * 60000,
      );
      ...
      session = new AssessmentSession({
        ...
        endsAt,
        status: "in_progress",
        timeLimitMinutes: test.durationMinutes || 0,
        ipAddress: req.ip,
    ...
    const updatedAccess = await buildAssessmentAccess(test, req.user.userId);

    res.status(200).json({
      success: true,
      message: "Assessment session is active",
      data: updatedAccess,
    });
  } catch (error) {
...
export const completePublicAssessmentSession = async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: "Failed to complete assessment session",
      error: error.message,
    });
  }
};
...
export const getTestStats = async (req, res) => {
  try {
    ...
    const stats = {
      ...
      draft: await Test.countDocuments({
        creatorId: facultyId,
        status: "draft",
      }),
      published: await Test.countDocuments({
        creatorId: facultyId,
        status: "published",
      }),
      archived: await Test.countDocuments({
        creatorId: facultyId,
        status: "archived",
      }),
      byType: {
        assessment: await Test.countDocuments({
          creatorId: facultyId,
          type: "assessment",
        }),
        assignment: await Test.countDocuments({
          creatorId: facultyId,
          type: "assignment",
        }),
        contest: await Test.countDocuments({
          creatorId: facultyId,
          type: "contest",
        }),
      },
